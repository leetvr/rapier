...
pub struct UniversalArticulation {
    revo1: RevoluteArticulation,
    revo2: RevoluteArticulation,

    coupling_dot: RigidBodyVelocity,
    coupling_dot_veldiff: RigidBodyVelocity,
}

impl UniversalArticulation {
    /// Creates an universal articulation allowing rotations along `axis1` and `axis2`.
    ///
    /// The axes are expressed in the local coordinate systems of the attached multibody links.
    pub fn new(
        ...
        UniversalArticulation {
            revo1: RevoluteArticulation::new(axis1, angle1),
            revo2: RevoluteArticulation::new(axis2, angle2),
            coupling_dot: RigidBodyVelocity::zero(),
            coupling_dot_veldiff: RigidBodyVelocity::zero(),
        }
    }
}

impl Articulation for UniversalArticulation {
    #[inline]
    fn ndofs(&self) -> usize {
        2
    }
    ...
    fn update_jacobians(&mut self, body_shift: &Vector3<Real>, vels: &[Real]) {
        ...
        self.coupling_dot_veldiff =
            RigidBodyVelocity::new(axis1.cross(&jac2.linvel), axis1.cross(&jac2.angvel));
...
revolute_motor_limit_methods_1!(UniversalArticulation, revo1);
revolute_motor_limit_methods_2!(UniversalArticulation, revo2);
