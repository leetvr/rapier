...
/// A articulation that allows one translational and one rotational degrees of freedom along a single axis.
#[derive(Copy, Clone, Debug)]
pub struct CylindricalArticulation {
    prism: PrismaticArticulation,
    revo: RevoluteArticulation,
}

impl CylindricalArticulation {
    /// Create a cylindrical articulation with the given axis and initial position of angle.
    ///
    /// The axis is expressed in the local space of the multibody links attached to this articulation.
    pub fn new(axis: Unit<Vector3<Real>>, position: Real, angle: Real) -> Self {
        let prism = PrismaticArticulation::new(axis, position);
        let revo = RevoluteArticulation::new(axis, angle);

        CylindricalArticulation { prism, revo }
    }
}

impl Articulation for CylindricalArticulation {
    #[inline]
    fn ndofs(&self) -> usize {
        2
    }
...
prismatic_motor_limit_methods!(CylindricalArticulation, prism);
revolute_motor_limit_methods!(CylindricalArticulation, revo);
